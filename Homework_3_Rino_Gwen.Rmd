...
1. The problem with this graph is that it is overplotted (points overlap each other). It could be improved by using `position = "jitter"` to add random noise that will spread the points out a bit and make them easier to see and interpret.
...
4. This plot shows several things: there is a positive linear relationship between city mpg and highway mpg, all cars are more fuel efficient on the highway than in the city, and there is (generally) a greater difference between city and highway fuel efficiency for cars that get higher mpg in the city (that is, the slope of the best fit line is greater than 1).
