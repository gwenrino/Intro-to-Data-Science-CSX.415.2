...
The second visualization (below) suggests that as the average delay increases for a particular day, so does the number of cancelled flights. This makes intuitive sense!
...
2. The data frame subsetting `df$x` extracts the data in column `xyz`, whereas the tibble subsetting `tb$x` returns an error because it distinguishes `x` from `xyz`.
```{r}
...
2. The code fails because the column names "1999" and "2000" are non-syntactic names and therefore must be surrounded by backticks.

3. Spreading the given tibble fails because "Phillip Woods" has two different values for "age", presumably because there are two different people in this data set with the same name. You could solve this problem by adding a new column that distinguishes Phillip Woods #1 from Phillip Woods #2. See code below:
...
1. When `sep` is a character vector, `extra` controls what happens when there are too many pieces and `fill` controls what happens when there aren't enough pieces. See below for examples:
...
# Fill = "warn" (default) gives a warning and fills from the right
tibble(x = c("a,b,c", "d,e", "f,g,i")) %>% 
  separate(x, c("one", "two", "three"))
# Fill = "right" does the same thing without a warning

# Fill = "left" fills with missing values from the left
tibble(x = c("a,b,c", "d,e", "f,g,i")) %>% 
  separate(x, c("one", "two", "three"), fill = "left")
```

2. The default for `remove` in `unite()` and `separate()` is TRUE. This removes from the output data frame the original columns that were transformed by the function. You would set it to FALSE if you wanted both the original columns and the transformed ones in your output data frame, as in the example below:
